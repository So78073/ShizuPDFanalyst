...
def create_pdf_with_color_pages(pdf_path, output_pdf_path_color, output_pdf_path_bw, mode=(True, False), result_container=None, index=None):
    try:
        ...
        if mode[0]:  # Salvar PDF colorido
            if output_pdf_path_color:
                output_dir = os.path.dirname(output_pdf_path_color)
                if output_dir and not os.path.exists(output_dir):
                    os.makedirs(output_dir)
                pdf_color.save(output_pdf_path_color)
                print(f"PDF com páginas coloridas salvo em {output_pdf_path_color}")

        if mode[1]:  # Salvar PDF preto e branco
            if output_pdf_path_bw:
                output_dir = os.path.dirname(output_pdf_path_bw)
                if output_dir and not os.path.exists(output_dir):
                    os.makedirs(output_dir)
...
def main(threshold, pdf_path, output_pdf_path_color, output_pdf_path_bw, mode=(True, False), mods=(False, False, False)):
    if not os.path.isfile(pdf_path) or not pdf_path.endswith('.pdf'):
        print(f"Erro: o arquivo '{pdf_path}' não é um arquivo PDF válido. Verifique o caminho e a extensão do arquivo.")
        return
    ...
    print(f"Resultados finais: {result_container}")
